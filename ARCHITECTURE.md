...
| `worldgen/configured_feature` | `lit_spaceships:planetary_ring` | `ModConfiguredFeatures` | → Runtime-Feature `lit_spaceships:planetary_ring` (`ModFeatures`, `PlanetaryRingFeature`) |
| `worldgen/configured_feature` | `lit_spaceships:asteroid_belt` | `ModConfiguredFeatures` | → Runtime-Feature `lit_spaceships:asteroid_belt` (`ModFeatures`, `AsteroidBeltFeature`) |
| `worldgen/placed_feature` | `lit_spaceships:asteroid_placed` | `ModPlacedFeatures` | → `lit_spaceships:asteroid`; Count 1 (Rebalance 2026-09-11, vorher 4 — mehr leerer Weltraum), InSquare, Uniform $Y \in [-40, 280]$, Biome-Filter |
...
| `worldgen/placed_feature` | `lit_spaceships:wreck_field_placed` | `ModPlacedFeatures` | → `lit_spaceships:space_wreck` (Feature-Wiederverwendung); Rarity 1/10 (Rebalance 2026-09-11, vorher 1/4), InSquare, Uniform $Y \in [0, 200]$, Biome-Filter |
...
| `worldgen/structure` | `lit_spaceships:the_monolith` | `ModStructures::bootstrapStructure` | `JigsawStructure` (Start-Pool `the_monolith/start`, maxDepth 1, Uniform $Y \in [96, 240]$, TerrainAdjustment NONE); `space_biome` & `plasma_nebula` |
...
| `worldgen/configured_feature` | `lit_spaceships:cosmic_jellyfish` | `ModConfiguredFeatures` | → Runtime-Feature `lit_spaceships:cosmic_jellyfish` (`ModFeatures`, `CosmicJellyfishFeature`) |
| `worldgen/placed_feature` | `lit_spaceships:cosmic_jellyfish_placed` | `ModPlacedFeatures` | → `lit_spaceships:cosmic_jellyfish`; Count 1 + InSquare + Biome-Filter (Zellen-Spec 35%, Y 96-288). Glas-Schirm R 4-6 mit Herz-Kiste `chests/jelly_heart` — erste Features der `plasma_nebula` |
| `worldgen/structure` | `lit_spaceships:colony_dome` | `ModStructures::bootstrapStructure` | `JigsawStructure` (Start-Pool `colony_dome/start`, maxDepth 1, Uniform $Y \in [64, 200]$, TerrainAdjustment NONE); `space_biome` & `frozen_expanse` |
| `worldgen/structure_set` | `lit_spaceships:colony_dome` | `ModStructures::bootstrapStructureSet` | `RandomSpreadStructurePlacement` (Spacing 50, Separation 18, LINEAR, Seed 223356207) |
| `worldgen/configured_feature` | `lit_spaceships:ancient_battlefield` | `ModConfiguredFeatures` | → Runtime-Feature `lit_spaceships:ancient_battlefield` (`ModFeatures`, `AncientBattlefieldFeature`) |
| `worldgen/placed_feature` | `lit_spaceships:ancient_battlefield_placed` | `ModPlacedFeatures` | → `lit_spaceships:ancient_battlefield`; Count 2 + InSquare + Biome-Filter (Zellen-Spec 30%, Y 0-240). Verkohlte Cluster mit Bergungs-Kisten `chests/battlefield_salvage` (15%) |
| **Epoch 8-11** | 3 Extreme-Biome (`gravity_rift`/`stellar_corona`/`ion_storm`) | `ModBiomes` + `ModNoiseSettings` (4-Achsen-Router T/H/C/W) | 7-Biome-Partition: extreme Zonen schneiden die auf C[-0.85,1]×W[-0.65,1] eingeschraenkte Basis-Partition heraus — gapless, ohne Tie-Breaking |
| **Epoch 8-11 Makro** | `GravityRiftFeature`/`StellarCoronaFeature`/`IonStormFeature` | `ModConfiguredFeatures` + `ModPlacedFeatures` | Akkretionsscheibe (2048er-Zellen, Baender Crying Obsidian/Gilded Blackstone), Flare-Herde (1024er-Zellen), EMP-Pylone (2048er-Zellen) — alle deterministisch + chunk-lokal |
| **Epoch 9 Runtime** | `DecompressionService` + `BiomeAtmosphereService` Erweiterung | `@EventBusSubscriber` | pressurized_hull-Brechung: 5x5x5-Kavernenscan + 60-Tick-Auswaerts-Sog; Gravity Shear (Sog zur Mitte), Solarstrahlung (Gold/Netherite schuetzt, Ruestungsabnutzung), Ionstoerung (Uebelkeit/Elytra-Stall/Blitze) — alle Planner pure + statistisch getestet |
| **Epoch 10 Telemetrie** | `Telemetry` + `SignalScopeItem` + `BeaconBlock(Entity)` + `ModDataComponents` | Items/BE | Alignment = L·T (normiert), Ping-Pitch 0.5-1.3, Daempfung bis 512 Blöcke, XOR-Koordinaten, 3 Frequenz-Kanaele, TelemetryData-Component (persistent + synchronisiert) |
| **Epoch 12 Salvage** | `ReactorMeltdownLogic` (pure) + `UnstableReactorBlock(Entity)` + `CutPointBlock` + `SealedSalvageCrateBlock` + `AccessCipherItem` | Blocks/BE | 90s-Meltdown-Countdown (Stufe 0-4), 2 Kuehlungen = Reset + reactor_core_salvage-Freischaltung, sonst Explosion (Power 4); 3x3x3-Panel-Logik; Redstone/Cipher-Entsiegelung |
| **Runtime** | `BiomeAtmosphereService` | `@EventBusSubscriber` (game bus) | `PlayerTickEvent.Post` alle 30 Ticks: deterministischer `planFor(biome, random)`-Plan (JUnit-statistisch verifiziert) → kurze Biom-Effekte (Nebula: Leuchten 5s / Sanfter Fall 15s, Frozen: Langsamkeit 4s, Void Wastes: Dunkelheit 8s, Deep Space: neutral) + i18n Action-Bar-Nachricht; kein Stapeln, Kreativ/Zuschauer ausgeschlossen. Biom-Soundscapes via `BiomeSpecialEffects` (Basalt-Loop / Seelenwind-Loop / Eis-Additions; Void Wastes stumm) |
| `worldgen/configured+placed` | 20 Ambient-Features (`ModAmbientFeatures`) | `ModAmbientFeatures::bootstrapConfigured/bootstrapPlaced` | 5 pro Biom aus 4 wiederverwendbaren Klassen: `ScatterBlockFeature` (gewichtetes Block-Streuen), `PillarFeature` (Säule + Cap), `OrbFeature` (R=2 Kugel, Hülle+Kern), `PodFeature` (2x2x2 Frachtpod mit `chests/cargo_pod`). Deep Space: debris_field, meteor_shower, void_crystal_spike, beacon_pylon, cargo_pod (1/24). Nebula: nebula_spore_drift, plasma_ember, nebula_gas_bloom, crystal_lattice, nebula_arc. Frozen: ice_shard_field, glacier_floe, frost_pillar, snow_bloom, cryo_geode. Void Wastes: bone_debris, scrap_wasteland, dust_drift, ash_vent, void_cyst |
