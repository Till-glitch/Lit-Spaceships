...
public class WorldGenGameTests {
    ...
    public static void extremeBiomesRegisteredWithSignatureVisuals(GameTestHelper helper) {
        ...
        if (rift.getFogColor() != 0x0B001A || corona.getFogColor() != 0xFF4500
                || storm.getFogColor() != 0x1E90FF) {
            helper.fail("Extreme-Biome haben falsche Signature-Farben");
            return;
        }
        if (rift.getAmbientParticle().isEmpty()
                || corona.getAmbientParticle().isEmpty()
                || storm.getAmbientParticle().isEmpty()) {
