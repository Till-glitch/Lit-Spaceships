...
* **Deep Space Dimension (`lit_spaceships:space`):**
  ...
  * **Frozen Expanse (Volumetric 3D Biome):** Pale cyan atmosphere (`#00FFFF`) with falling snowflake particle streams and high-density comet fields: floating ice comets with rare blue-ice cores, packed-ice mantles, and icy crusts (`IceCometFeature`, 8 placement attempts per chunk). Zero mob spawns.
  * **Void Wastes (Volumetric 3D Biome):** Total sensory deprivation — complete blackness (fog/sky `#000000`) with no ambient particles or cave sounds at all — and a derelict graveyard: normal asteroid fields plus a dense wreck field (1 wreck per 4 chunks, 8× the base spawn rate) reusing the `space_wreck` feature. Zero mob spawns.
  * **Mega Hollow Asteroids (`MegaAsteroidFeature`):** Colossal 3D ellipsoids (40–70 block diameter, radii 20–35) floating in the void: rocky crust over an ore-injected mantle (diamond, ancient debris, raw gold, iron, copper, redstone), a completely hollow cavern interior, and a floating amethyst geode at the core (calcite shell, amethyst-block mantle with budding amethyst, hollow geode chamber). Extremely rare (~1 per 96 chunks) to respect chunk-generation budgets. Verified in-world by a GameTest on a scaled-down ellipsoid.
  * **Planetary Rings (`PlanetaryRingFeature`):** Vast horizontal ring systems (radius 100–300 blocks, 1–3 blocks thick) of ice, stained glass, and dust blocks sweeping across the frozen, void, and base space biomes. Each 2048-block region deterministically derives one ring whose center is clamped fully inside the region — every chunk places only its own 16×16 column segment, giving seamlessly connected rings with zero cross-chunk writes and negligible generation cost. Verified in-world by a GameTest on a scaled-down annulus.
  * **Dense Asteroid Belts (`AsteroidBeltFeature`):** Procedural belt corridors per 1024-block region (length 320–640 blocks, width 16–32, Y −32..256) placing variable-size ore-bearing rock fragments (radius 2–6) with sinusoidal cluster noise — dense clumps and gaps along the belt axis. Fragment cores carry iron, copper, redstone, raw gold, diamonds, and rare ancient debris. Verified in-world by a GameTest on a scaled-down fragment.
  * **Asteroid Fields & Ice Comets:** 3D procedural asteroid generation with diverse crusts (Stone, Basalt, Tuff, Deepslate) and rare ore cores (Diamond ~3%, Ancient Debris ~2%, Gold, Iron) — the void stays vast and mostly empty; the real riches hide in belts, mega-asteroids and derelicts. Frozen ice comets sweep through the Frozen Expanse.
  * **Abandoned Orbital Research Station (`lit_spaceships:space_station`):** Modular procedural Jigsaw space station (Docking Hub, Solar Wing corridor with solar paneling, Laboratory, and Reactor Room). Features custom loot table `lit_spaceships:chests/space_station_core` containing weapon smithing templates, high-capacity redstone battery blocks, diamonds, and tech components. Distributed via Random-Spread (Spacing 36, Separation 12) across all four space biomes.
  * **Derelict Dreadnought Warship (`lit_spaceships:dreadnought_wreck`):** Colossal ruptured naval capital ship floating in the void. Consists of a heavily armored Command Bridge with red glass visors, breached corridors with hull punctures open to the cosmic vacuum, and an Engineering Core with an unstable containment reactor (netherite, magma, crying obsidian). Houses the naval armory chest (`lit_spaceships:chests/dreadnought_armory`) with Netherite scrap, Echo Shards (pulse laser components), blaze rods, and naval munitions. Distributed via Random-Spread (Spacing 48, Separation 16) across the Deep Space base biome and the Void Wastes derelict graveyard.
  * **Alien Monolith Relic (`lit_spaceships:alien_outpost`):** Ancient geometric monolith structure of smooth and polished basalt, purpur columns topped with end rods, and a central spire anchoring a mysterious Lodestone core surrounded by sea lanterns and amethyst crystals. Holds an ancient relic chest (`lit_spaceships:chests/alien_monolith`) containing lodestones, rare nether stars, echo shards, ender eyes, and exotic void materials. Distributed via Random-Spread (Spacing 40, Separation 14) across Deep Space and Plasma Nebula biomes.
  * **Cosmic Vault (`lit_spaceships:cosmic_vault`):** The rarest structure of the void — a fully sealed obsidian cube with a single cyan view-port revealing a treasure pedestal inside. The vault chest (`lit_spaceships:chests/cosmic_vault`) holds endgame rewards: enchanted golden apples, netherite ingots, end crystals, and totems of undying. Distributed via Random-Spread (Spacing 64, Separation 20) across all four space biomes — finding one is a genuine discovery.
  * **Pirate Satellite Outpost (`lit_spaceships:pirate_outpost`):** A ramshackle blackstone platform on stilts, lit by soul lanterns, left behind by void raiders. The loot cache (`lit_spaceships:chests/pirate_cache`) — crossbows, golden apples, emeralds, gunpowder — sits on a booby-trapped floor: the TNT is directly beneath the chest. Distributed via Random-Spread (Spacing 44, Separation 16) across Deep Space and the Void Wastes.
  * **Frozen Leviathan (`lit_spaceships:leviathan_bones`):** The colossal skeleton of a void beast drifts through the Frozen Expanse — a 17×21-block bone ribcage with a blue-ice skull and a magma heart. Crack the hoard chest at its heart (`lit_spaceships:chests/leviathan_hoard`) for a Heart of the Sea, diamonds and ancient ice. Frozen Expanse only, Random-Spread (Spacing 52, Separation 18).
  * **The Silent Monolith (`lit_spaceships:the_monolith`):** A pitch-black obsidian slab standing on a drifting shard of deepslate rock, marked only by two faint end rods. It hides a secret: a sealed chamber buried inside the rock beneath the base, holding offerings and a chest (`lit_spaceships:chests/monolith_secret`) with netherite scrap, enchanted golden apples, echo shards and eyes of ender. Nothing hints at it — the reward belongs to the curious. Deep Space and Plasma Nebula, Random-Spread (Spacing 56, Separation 20).
...
* **Deep Space ambient features:** Debris fields, meteor showers, void crystal spikes, beacon pylons and rare cargo pods (rarity 1/24, `lit_spaceships:chests/cargo_pod` with a chance of a music disc) litter the void.
  ...
  * **Abandoned Colony Dome (`lit_spaceships:colony_dome`):** A settler outpost that lost its people — a glass dome with a still-burning campfire, supply barrels and a larder chest (`lit_spaceships:chests/colony_larder`) full of bread, roasts and hay. Amethyst has begun to overgrow the floor tiles. Deep Space and Frozen Expanse, Random-Spread (Spacing 50, Separation 18).
  * **Gravity Rift (`lit_spaceships:gravity_rift`):** Extreme biome at the edge of the void — near-black fog with reverse-portal particles and basalt sub-bass. Home to a deterministic **accretion disk** of crying obsidian and gilded blackstone around a per-2048-cell singularity, plus the Deep Listening Outpost (hollow asteroid with anomaly beacon and archive chest).
  * **Stellar Corona (`lit_spaceships:stellar_corona`):** Orange solar chromosphere with flame particles. Solar flares of magma and lava cauldrons dot the region, alongside the **Solar Collector** thermal platform (solar_harvest loot). Solar radiation hurts unprotected players — gold or netherite armor protects.
  * **Ion Storm (`lit_spaceships:ion_storm`):** Electric-blue charged rift valleys with spark particles. EMP pylons of copper with lightning rods and chain webs; compasses and elytra stall under ion interference. Home of the **Relay Array** antenna lattice (relay_intercept loot).
  * **Biome Identity — Soundscapes & Atmosphere Effects (`BiomeAtmosphereService`):** Every biome has its own soundscape and mild, short biome-effects: Deep Space rumbles like basalt deltas; the Plasma Nebula breathes with soul-wind and ionizes your suit (**Glowing** 5s) or lifts you (**Slow Falling** 15s); the Frozen Expanse whispers and bites with **hypothermic slowness** (4s); the Void Wastes stay perfectly mute but press **Darkness** pulses (8s) on your eyes. Effects are rare, never stack, skip creative/spectator players — announced via action-bar messages (en/de).
  * **Behemoth Freighter (`lit_spaceships:behemoth_freighter`):** 4-module jigsaw cargo ship (~84 blocks: bridge → cargo bays → fractured corridor → engineering bay) with behemoth_manifest loot. Deep Space + Void Wastes, Random-Spread (Spacing 56, Separation 20).
  * **Salvage Engineering:** **Unstable Reactor** cores run a 90-second meltdown countdown — two coolant resets unlock the reactor_core_salvage chamber; failure explodes. **Alloy Cut-Points** drop concentrated scrap when their panel is fully cut. **Sealed Salvage Crates** open only with redstone current or an Access Cipher. **Signalscope** scans 512 blocks for transponder beacons (pitch-modulated pings); **Flight Recorders** decrypt translated ship logs with coordinate clues.
  * **Ancient Battlefield (`lit_spaceships:ancient_battlefield`):** Scorched debris fields from a war nobody remembers — craters of obsidian and smoldering magma, blackstone hulks trailing chains. Some hulks still carry salvage chests (`lit_spaceships:chests/battlefield_salvage`) with swords, shields, crossbows, TNT and netherite scrap. Deep Space and Void Wastes.
  * **Derelict Spacecraft Wrecks:** Rare abandoned shipwrecks featuring intact spaceship reactor cores and ancient treasure chests.
  * **Custom Deep Space Economy & Chest Loot Tables (`ModChestLootTableProvider`):** Data-generated custom loot tables (`space_station_core`, `dreadnought_armory`, `alien_monolith`) integrating exploration directly with the mod's weapon smithing and navigational ship core progression.
...
* **Automated Data Generation Pipeline (`com.lit.spaceships.datagen`):**
  ...
  * **Server DataGen (Domain Layer):** `ModWorldGenProvider` coupling a `RegistrySetBuilder` (Biomes, ConfiguredFeatures, PlacedFeatures, TemplatePools, Structures, StructureSets) into `DatapackBuiltinEntriesProvider`, making the Deep Space Dimension 100% code-generated with zero hand-written worldgen JSON; `ModLootTableProvider` with `ModBlockLootTableProvider` (block self-drops) and `ModChestLootTableProvider` (custom space chest loot tables); `ModRecipeProvider` for compile-time verified recipe and advancement generation.
