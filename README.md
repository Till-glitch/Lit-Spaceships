...
* **Spaceship Controller (Core Lifecycle & Structure Management Terminal):**
  ...
  * **Subsystem Registry Summary:** Real-time itemized overview of linked functional blocks (Reactors, Shield Generators, Heavy Beams, Pulse Lasers, Mining Lasers, Navigation Helms, and Warp Engine Cores) both for bound ships and unbound pre-flight scans.
...
* **Warp Engine & Interdimensional Transit (`lit_spaceships:warp_engine`):**
  ...
  * **Internal Energy Storage & Dynamic Cost Scaling:** Base jump requirement of 100,000 FE plus an additional 10 FE per connected block of the ship ($100{,}000\text{ FE} + N_{\text{blocks}} \times 10\text{ FE}$). Holds up to 2,000,000 FE internal capacity to accommodate small vessels and capital dreadnoughts alike. Accepts standard Forge Energy and automatically trickle-charges at 500 FE/t from connected shipboard reactor cores. When starting a jump, any deficit beyond the base 100,000 FE can be drawn directly from the ship's reactor grid.
  * **Sci-Fi Terminal UI (`WarpEngineScreen`):** Light-blue and neon-cyan themed visual console with real-time FE gauge, dynamic status badge (`READY`, `CHARGING`, `COUNTDOWN`, `COOLDOWN`, `UNLINKED`), destination indicator, vessel telemetry with detailed warp cost breakdown (`Base: 100k + X FE for Y blocks`), and interactive controls.
  ...
  * **Calibrated Terrestrial Arrival Altitude & Surface Clearance:** When warping back to the Overworld, computes arrival altitude relative to the true bottom of the ship's bounding box (`bottomOffset`), safely hovering approximately 4 blocks above the highest ground/tree canopy (`surfaceY + bottomOffset + 4`). Employs adaptive vertical stepping (`findSafeElevationAt`) to seamlessly fly over tree canopies and obstacles without displacing the ship into high altitude.
* **Cross-Dimensional Ship Travel & Robust Lifecycle Retention (`ShipTeleportationService`):**
  ...
  * **Ghost-Ship & Excision Protection:** Prevents `SpaceshipControlBlock.onRemove` from inadvertently deleting the ship entity during warp excision (`isJumping()`), guarantees continuous registration in `ServerShipManager.ACTIVE_SHIPS`, cleanses block entities prior to excision to prevent inventory drops, and immediately syncs newly materialized ships to players upon dimension transition (`PlayerChangedDimensionEvent`).
  * **Seamless Controller & Client State Continuity:** Retains ship states in client memory across dimension shifts while safely disposing native OpenGL VBO buffers; automatically updates and binds the Spaceship Controller screen so operational controls (Update Blocks, Disassemble, Rotate CW/CCW) remain fully active and "Create Ship" remains disabled post-warp.
